...
class Scheduler:
    ...
    def add_job(self, func, trigger, **kwargs):
        ...
        try:
            ...
            logging.info(f"Added a job: {func.__name__} with {trigger} trigger and {kwargs} arguments.")
    ...
    def shutdown(self):
        ...
        try:
            ...
            logging.info("Shutdown the scheduler successfully.")
        except Exception as e:
            logging.error(f"Failed to shutdown the scheduler: {e}")
    ...
    def pause(self): \
        ...
        try:
            ...
            logging.info("Paused the scheduler successfully.")
        except Exception as e:
            logging.error(f"Failed to pause the scheduler: {e}")

    def resume(self):
        # 调用调度器的resume方法，恢复调度器
        try:
            self.SCHEDULER.resume()
            logging.info("Resumed the scheduler successfully.")
        except Exception as e:
            logging.error(f"Failed to resume the scheduler: {e}")

    def modify_job(self, job_id, **changes):
        # 调用调度器的modify_job方法，修改某个任务的属性
        try:
            self.SCHEDULER.modify_job(job_id, **changes)
            logging.info(f"Modified a job: {job_id} with {changes} changes.")
        except Exception as e:
            logging.error(f"Failed to modify a job: {e}")
